...
func getFilePath(jobID string) string {
	...
	dirPath := "/tmp/automation/memory/file_store/"
	if err := os.MkdirAll(dirPath, 0755); err != nil && !os.IsExist(err) {
		fmt.Printf("error creating directory structure: %s\n", err)
		return ""
	}
	return "/tmp/automation/memory/file_store/" + jobID
}

func AddChatHistory(jobID, automationID string, chatHistory []automations.MessageDataDtoV1) error {
...
func AddMessage(jobID, automationID, content string, messageType message_enums.Type) error {
