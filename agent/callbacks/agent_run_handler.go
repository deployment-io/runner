...
// AgentRunHandler is a callback handler used for observing and logging LLM runs for an agent.
...
var _ callbacks.Handler = &AgentRunHandler{}

func (a *AgentRunHandler) HandleToolStart(_ context.Context, info string) {
	if a.Debug {
		a.Lock()
		defer a.Unlock()
...
func (a *AgentRunHandler) HandleToolEnd(_ context.Context, info string) {
	if a.Debug {
		a.Lock()
		defer a.Unlock()
		io.WriteString(a.LogsWriter, fmt.Sprintf("%s\n", removeNewLines(info)))
...
func (a *AgentRunHandler) HandleToolError(_ context.Context, err error) {
	if a.Debug {
		a.Lock()
		defer a.Unlock()
...
func (a *AgentRunHandler) HandleLLMError(_ context.Context, err error) {
...
func (a *AgentRunHandler) HandleLLMGenerateContentStart(_ context.Context, ms []llms.MessageContent) {
	if a.DebugOpenAICalls {
		a.Lock()
		defer a.Unlock()
...
func (a *AgentRunHandler) HandleLLMGenerateContentEnd(_ context.Context, res *llms.ContentResponse) {
	if a.DebugOpenAICalls {
		a.Lock()
		defer a.Unlock()
...
func (a *AgentRunHandler) HandleStreamingFunc(ctx context.Context, chunk []byte) {
}
